local M = {}

local ext_to_lang = {
	zig = "zig",
	lua = "lua",
	c = "c",
	h = "c",
	rs = "rust",
	py = "python",
	js = "javascript",
	ts = "typescript",
}

local home = os.getenv("HOME")
local parsers_dir = home .. "/.config/dot/parsers/"
local queries_dir = home .. "/.config/dot/queries/"

local current_loaded_lang = nil

local function update_tree()
	local active = dot.buf.active()
	local filename = active and active:name() or ""
	if not filename or filename == "" then
		return false
	end

	local ext = filename:match("^.+(%..+)$")
	if not ext then
		return false
	end
	ext = ext:sub(2)

	local lang_name = ext_to_lang[ext]
	if not lang_name then
		return false
	end

	if current_loaded_lang == lang_name then
		return false
	end

	local lib_path = parsers_dir .. lang_name .. ".so"
	local query_path = queries_dir .. lang_name .. "/highlights.scm"

	local f = io.open(lib_path, "r")
	if f then
		f:close()
		dot.ui.notify("🌳 Tree-sitter loaded : " .. lang_name)
		dot.ts.load(lang_name, lib_path, query_path)
		current_loaded_lang = lang_name
	else
		dot.ui.notify("lang not found: use lang-install <lang>")
	end

	return false
end

dot.hook.on("BufInit", update_tree)
dot.hook.on("BufOpen", update_tree)
dot.cmd.create("ts", update_tree)

return M